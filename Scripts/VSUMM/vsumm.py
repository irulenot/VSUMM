...
def main():
	...
	frames=[video.get_data(i*sampling_rate) for i in range(int(video.count_frames()/sampling_rate))]
	print("Frames chosen")
	print("Length of video %d" % video.count_frames())

	# converting percentage to actual number
	num_centroids=int(percent*video.count_frames()/100)
	if (video.count_frames()/sampling_rate) < num_centroids:
		print("Samples too less to generate such a large summary")
		print("Changing to maximum possible centroids")
		num_centroids=video.count_frames()/sampling_rate
